-- Linux Wayland window snapshot using wlr foreign-toplevel v1; no game policy.
-- snapshot() returns { {app_id, active}, ... }, or nil when unavailable.
local M = {}
...
if jit then jit.off(snapshot, true) end

function M.snapshot()
    local ok, windows, err = pcall(snapshot)
    if not ok then return nil, tostring(windows) end
    return windows, err
end
